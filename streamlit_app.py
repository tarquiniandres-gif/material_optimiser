...
def normalise(text):
    ...
    text = text.replace("X", "X").replace("Ø", "⌀")
    return text
    ...
    for desc_norm, group in grouped:
        ...
        if std_length is None:
            buy_rows.append({
                ...
                "_warning": f"No standard length defined. Please set it above."
            })
            continue
...
with col2:
    num_turntables = st.number_input("Number of turntables to produce", min_value=1, value=1, step=1)
with col3:
...
if process_btn:
    if paste_df is None and uploaded_df is None:
        ...
        if not set(required) <= set(col_map.keys()):
            ...
            # --- Apply turntable multiplier ---
            try:
                df_proc["qty"] = df_proc["qty"].astype(float).astype(int) * int(num_turntables)
            except:
